...
#SBATCH --mem=60G
#SBATCH -t 24:00:00
#SBATCH --output=slurm_rlaf_3col_nolsp_%j.out
#SBATCH --error=slurm_rlaf_3col_nolsp_%j.err
#SBATCH --mail-type=BEGIN,END,FAIL
#SBATCH --mail-user=YOUR_EMAIL@example.com

# ─── Snellius RLAF Training: no COOC, no local-satisfaction features ───────
# Same as job_rlaf_3col.sh but with model.no_precomputed_local_sat=true.
# The model receives zeros for the local-satisfaction-percentage edge feature
# at every round.  No co-occurrence edges.
#
# Cache: use_cooc=false → dataset reads from processed_rlaf/ (same cache as
# the regular no-COOC run — no extra preprocessing needed).
# ──────────────────────────────────────────────────────────────────────────
...
# ── Train (no COOC, no precomputed local sat) ──────────────────────────────
python -u train_arienet_rlaf.py \
    -n ArieNet_3col_noLSP \
    method=grpo \
    use_cooc=false \
