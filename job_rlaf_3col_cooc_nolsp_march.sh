#!/bin/bash
#SBATCH -J rlaf_3col_cooc_nolsp_march
#SBATCH -p gpu_a100
#SBATCH -N 1
#SBATCH --ntasks=1
...
#SBATCH --mem=60G
#SBATCH -t 24:00:00
#SBATCH --output=slurm_rlaf_3col_cooc_nolsp_march_%j.out
#SBATCH --error=slurm_rlaf_3col_cooc_nolsp_march_%j.err
#SBATCH --mail-type=BEGIN,END,FAIL
#SBATCH --mail-user=YOUR_EMAIL@example.com

# ─── Snellius RLAF Training: COOC, no LSP, march solver ────────────────────
# Identical to job_rlaf_3col_cooc_nolsp.sh except solver=march.
# Cache: processed_rlaf_cooc/ (shared with regular COOC runs).
# ──────────────────────────────────────────────────────────────────────────
...
python -u train_arienet_rlaf.py \
    -n ArieNet_3col_COOC_noLSP_march \
    method=grpo \
    use_cooc=true \
    ...
    wandb.name=ArieNet_3col_COOC_noLSP_march
