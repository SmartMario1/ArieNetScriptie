...
from nsnet.utils.utils import write_dimacs_to, VIG
...
class Generator:
    ...
    def run(self, t):
        while True:
            ...
            # Only write satisfiable instances????
            if solver.solve():
...
def main():
    ...
    with ProcessPoolExecutor(max_workers=opts.n_process) as pool:
        results = pool.map(generater.run, range(opts.n_instances))
        for i, _ in enumerate(results):
            if (i + 1) % 10 == 0:
                print(f'Generated {i + 1}/{opts.n_instances}', end='\r')
