...
def main():
    ...
    opts.n_process = 4 if opts.task == 'marginal' else 6
    ########
    ...
    # Instead of accumulating all labels in memory, we'll save incrementally
    labels = []
    ...
    for i, result in enumerate(results):
        ...
        # Free memory from result object after processing
        del result
        if opts.task == 'marginal':
            del marginal
        ...
        if (i + 1) % 10 == 0:
            gc.collect()
            print(f"Processed {i + 1}/{tot} files, {cnt} successful")
