...
class ArieNetCooc(ArieNet):
    """ArieNet extended with a literal co-occurrence (L2L) message passing step.

    Co-occurrence edges are the literal-literal edges produced by
    cnf_to_lcg_cooccurrence: two literal nodes are connected when they appear
    together in at least one clause.  Each such edge has its own learnable
    starting vector (cooc_edges_init) that is distinct from the literal-clause
    edge inits, letting the model distinguish the two relationship types.

    The BPG data object must carry cooc_src_indices / cooc_dst_indices (added by
    the modified BPGParamBuilder).
    """

    def __init__(self, opts):
        super().__init__(opts)
        # Separate learnable starting vector for co-occurrence edges.
        self.cooc_edges_init = nn.Parameter(torch.randn(1, self.opts.dim))
        # MLP that maps aggregated co-occurrence signal → delta for c2l edge features.
        self.l2l_update = MLP(self.opts.n_mlp_layers, self.opts.dim, self.opts.dim, self.opts.dim, self.opts.activation)
