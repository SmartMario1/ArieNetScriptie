...
from nsnet.utils.dataset import BPG, LCG, OldBPG, MatrixBPG
...
def n_true_formulas(v_prob, data, device):
    ...
    if isinstance(data[0], BPG):
        model = "arienet"
    elif isinstance(data[0], MatrixBPG):
        model = "matrixnet"
    elif isinstance(data[0], (LCG, OldBPG)):
        model = "nsnet"
...
def main():
    ...
    print('\n[DATA LOADING] Starting to load training data from:', opts.train_dir)
    print('[DATA LOADING] Train size:', opts.train_size)
    print('[DATA LOADING] Model:', opts.model)
    print('[DATA LOADING] Batch size:', opts.batch_size)
    print('[DATA LOADING] This may take a while if graphs need to be processed for the first time...')
    
    # Builds all the graphs
    train_loader = get_dataloader(opts.train_dir, opts, 'train', opts.train_size)
    print('[DATA LOADING] Training data loaded in: %f seconds' % (time.time() - start_processing_time))
    
    if opts.valid_dir is not None:
        print('[DATA LOADING] Starting to load validation data from:', opts.valid_dir)
        valid_start = time.time()
        valid_loader = get_dataloader(opts.valid_dir, opts, 'valid')
        print('[DATA LOADING] Validation data loaded in: %f seconds' % (time.time() - valid_start))
    else:
        valid_loader = None

    print('\n[DATA LOADING] Total data processing time: %f seconds' % (time.time() - start_processing_time))
    print('[DATA LOADING] Ready to start training!\n')
