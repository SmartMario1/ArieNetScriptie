#!/bin/bash
#SBATCH -J rlaf_3col_nolsp_march
#SBATCH -p gpu_a100
#SBATCH -N 1
#SBATCH --ntasks=1
...
#SBATCH --mem=60G
#SBATCH -t 24:00:00
#SBATCH --output=slurm_rlaf_3col_nolsp_march_%j.out
#SBATCH --error=slurm_rlaf_3col_nolsp_march_%j.err
#SBATCH --mail-type=BEGIN,END,FAIL
#SBATCH --mail-user=YOUR_EMAIL@example.com

# ─── Snellius RLAF Training: no COOC, no LSP, march solver ─────────────────
# Identical to job_rlaf_3col_nolsp.sh except solver=march.
# Cache: processed_rlaf/ (same as no-COOC glucose run).
# ──────────────────────────────────────────────────────────────────────────
...
python -u train_arienet_rlaf.py \
    ...
    wandb.name=ArieNet_3col_noLSP_march
