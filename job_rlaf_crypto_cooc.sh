...
#SBATCH --mem=60G
#SBATCH -t 24:00:00
#SBATCH --output=slurm_rlaf_crypto_cooc_%j.out
#SBATCH --error=slurm_rlaf_crypto_cooc_%j.err
#SBATCH --mail-type=BEGIN,END,FAIL
#SBATCH --mail-user=YOUR_EMAIL@example.com

# ─── Snellius RLAF Training: ArieNetCooc (with COOC), glucose solver, crypto ──
# Run WITH co-occurrence graph extension (ArieNetRLAFCooc) on crypto instances.
# ──────────────────────────────────────────────────────────────────────────
...
python -u train_arienet_rlaf.py \
    ...
    wandb.name=ArieNet_crypto_COOC
