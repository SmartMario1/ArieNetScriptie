...
from nsnet.models.ln_lstm_cell import LayerNormBasicLSTMCell
...
class NeuroSAT(nn.Module):
    ...
    def forward(self, data):
        ...
        for _ in range(self.opts.n_rounds):
            ...
            # NEW PYTORCH
            l2c_msg_aggr = torch.zeros(c_size, dtype=l2c_msg.dtype).scatter_reduce(0, c_edge_index, l2c_msg, reduce="sum")
            ...
            # NEW PYTORCH
            c2l_msg_aggr = torch.zeros(l_size, dtype=c2l_msg.dtype).scatter_reduce(0, l_edge_index, c2l_msg, reduce="sum")
        ...
        if self.opts.task == 'model-counting':
            ...
            ## NEW PYTORCH
            return torch.zeros(data.l_size.shape[0], dtype=l_logits.dtype).scatter_reduce(0, l_batch, l_logits, reduce="mean")
