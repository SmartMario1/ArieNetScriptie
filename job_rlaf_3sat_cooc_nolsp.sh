...
#SBATCH --mem=60G
#SBATCH -t 24:00:00
#SBATCH --output=slurm_rlaf_3sat_cooc_nolsp_%j.out
#SBATCH --error=slurm_rlaf_3sat_cooc_nolsp_%j.err
#SBATCH --mail-type=BEGIN,END,FAIL
#SBATCH --mail-user=YOUR_EMAIL@example.com

# ─── Snellius RLAF Training: ArieNetCooc, no local-satisfaction features, 3-SAT
# Same as job_rlaf_3sat_cooc.sh but with model.no_precomputed_local_sat=true.
# COOC edges are kept; lsppe values in the cache are ignored at runtime.
# ──────────────────────────────────────────────────────────────────────────
...
# ── Train (COOC, no precomputed local sat, glucose) ───────────────────────
python -u train_arienet_rlaf.py \
    -n ArieNet_3sat_COOC_noLSP \
    method=grpo \
    use_cooc=true \
    ...
    wandb.name=ArieNet_3sat_COOC_noLSP
