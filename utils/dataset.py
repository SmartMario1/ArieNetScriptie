...
class ClauseNode:
    ...
    def satisfied_by_dict(self, assignment_dct):
        assert all(abs(l) in assignment_dct for l in self.content)
        return any([(literal>0) == assignment_dct[abs(literal)] for literal in self.content])
...
class BPGParamBuilder():
    ...
    def __init__(self,
        ...
        if total_time > 1.0:
            print(f'[BPGParamBuilder] n_vars={len(all_vars)}, n_clauses={len(all_clauses)}, n_edges={len(edges)}')
            print(f'[BPGParamBuilder] Timing: setup={setup_time:.2f}s, c2l={c2l_time:.2f}s, l2c={l2c_time:.2f}s, local_sat={local_sat_time:.2f}s, total={total_time:.2f}s')
            if local_sat_time > 0.5 * total_time:
                print(f'[BPGParamBuilder] WARNING: local_satisfaction_percentage is the bottleneck ({local_sat_time/total_time*100:.1f}% of time)')

        self.params = BPGParams(
            n_clauses = len(all_clauses),
            n_literals = len(all_vars)*2,
    ...
    def edges_around(self, node: Union[ClauseNode,VariableNode]) -> typing.List[Edge]:
        if isinstance(node, VariableNode):
            return [edge for edge in self.edges if edge.variable_node == node]
        
        elif isinstance(node, ClauseNode):
            return [edge for edge in self.edges if edge.clause_node == node]
    ...
    def local_satisfaction_percentage(self, edge: Edge):
        ...
        var_assignments_for_neighbors = list(itertools.product([True, False], repeat=len(other_vars)))

        n_satisfying_assignments = 0
        for assignment in var_assignments_for_neighbors:
            assignment_dict.update({var: val for var, val in zip(other_vars, assignment)})

            # Check if the shared clauses are satisfied by the assignment
            if all([c.satisfied_by_dict(assignment_dict) for c in shared_clauses]):
                n_satisfying_assignments += 1

        return n_satisfying_assignments / len(var_assignments_for_neighbors)
...
class SATDataset(Dataset):
    ...
    def process(self):
        ...
        for file_path, label in zip(self.all_files, self.all_labels):
            ...
            # Print progress every 10 files and for first file
            if idx == 0 or (idx + 1) % 10 == 0:
                print(f'[DATASET PROCESS] Processing file {idx + 1}/{len(self.all_files)}: {os.path.basename(file_path)}')
            ...
            elif self.graph == 'OldBPG':
                bpg_params = OldTransform2BPG(n_vars, clauses, self.opts.task)
                data = OldBPG(*bpg_params)
                # data = self._transform2BPG(n_vars, clauses)
                # data = OldBPG(n_vars, clauses, self.opts.task)
            elif self.graph == 'BPG':
                bpg_params = BPGParamBuilder(clauses).params
                data = BPG(*bpg_params)
            elif self.graph == 'MatrixBPG':
                bpg_params = MatrixBPGParamBuilder(clauses).params
                data = MatrixBPG(*bpg_params)
            else:
                raise ValueError(f'Graph type {self.graph} not supported')
            ...
            # Print timing for first file and slow files (> 5 seconds)
            if idx == 0 or total_file_time > 5:
                print(f'[DATASET PROCESS] File {idx + 1} timing: parse={parse_time:.2f}s, transform={transform_time:.2f}s, save={save_time:.2f}s, total={total_file_time:.2f}s')
                print(f'[DATASET PROCESS]   (n_vars={n_vars}, n_clauses={len(clauses)})')
            
            idx += 1
        
        total_time = time.time() - start_process_time
        print(f'\n[DATASET PROCESS] Finished processing all {idx} files in {total_time:.2f} seconds')
        print(f'[DATASET PROCESS] Average time per file: {total_time/idx:.2f} seconds\n')
