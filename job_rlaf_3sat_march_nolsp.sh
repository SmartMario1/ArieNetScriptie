#!/bin/bash
#SBATCH -J rlaf_3sat_march_nolsp
#SBATCH -p gpu_a100
#SBATCH -N 1
#SBATCH --ntasks=1
...
#SBATCH --mem=60G
#SBATCH -t 24:00:00
#SBATCH --output=slurm_rlaf_3sat_march_nolsp_%j.out
#SBATCH --error=slurm_rlaf_3sat_march_nolsp_%j.err
#SBATCH --mail-type=BEGIN,END,FAIL
#SBATCH --mail-user=YOUR_EMAIL@example.com

# ─── Snellius RLAF Training: no COOC, no local-satisfaction features, march, 3-SAT
# Identical to job_rlaf_3sat_march.sh but with model.no_precomputed_local_sat=true.
...
# ── Train (no COOC, no precomputed local sat, march) ──────────────────────
python -u train_arienet_rlaf.py \
    -n ArieNet_3sat_march_noLSP \
    method=grpo \
    use_cooc=false \
    ...
    wandb.name=ArieNet_3sat_march_noLSP
